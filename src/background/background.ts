...
chrome.runtime.onConnect.addListener(function (port) {
	port.onMessage.addListener(async function (msg) {
		if (msg.type === 'usagePort') {
			addTranslationUsage(msg.isTranslationSuccessful);
			return;
		}
...
const addTranslationUsage = (successFulTranslation: boolean) => {
	chrome.storage.local.get(
		...
		(res) => {
			...
			if (res.translationUsages) {
				...
				if (newUsage.length >= constants.USER_LIMITS.free.dailyUsageLimit[2]) {
					chrome.storage.local.set({
